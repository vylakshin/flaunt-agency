...
OVERLAY_THEME_OPTIONS = [
    {
        'value': 'classic',
        'label': 'Классический',
        'description': 'Текущий дизайн overlay с тёмной стеклянной карточкой.',
    },
    {
        'value': 'neo',
        'label': 'Новый',
...
async def twitch_callback(request: Request, code: str = '', state: str = ''):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail='Не удалось завершить авторизацию Twitch. Попробуй войти ещё раз.')
    except httpx.HTTPError as exc:
        request.session.pop('oauth_state', None)
        logger.exception('Twitch OAuth request failed: %s', exc)
        raise HTTPException(status_code=502, detail='Twitch временно недоступен. Попробуй ещё раз.')
...
async def dashboard(request: Request):
    ...
    if is_irc_mode:
        chat_connected = twitch_irc_listener.is_channel_connected(user['twitch_user_id'])
        bot_is_moderator = chat_connected
        chat_status_text = '' if chat_connected else _irc_connection_warning()
        bot_status_online_label = 'IRC подключен'
        bot_status_offline_label = 'IRC не подключен'
    else:
