...
from .twitch_chat import twitch_listener
from .twitch_irc import twitch_irc_listener
from .web_db import (
...
OVERLAY_THEME_OPTIONS = [
    {'value': 'classic', 'label': 'Классический', 'description': 'Текущий дизайн overlay с тёмной стеклянной карточкой.'},
    {'value': 'neo', 'label': 'Новый', 'description': 'Новый контрастный дизайн с более яркой подачей и переработанной композицией.'},
]
...
def _is_irc_mode() -> bool:
    return settings.twitch_mode.strip().lower() == 'irc'


def _get_chat_listener():
    return twitch_irc_listener if _is_irc_mode() else twitch_listener


def _irc_connection_warning() -> str:
    channel = settings.irc_target_channel or 'your_channel'
    return (
        f'IRC бот пока не подключился к чату #{channel}. '
        'Проверь BOT_LOGIN, BOT_OAUTH и TARGET_CHANNEL. '
        'После разрыва бот переподключается автоматически.'
    )


async def _activate_chat_for_user(user: dict) -> str:
    listener = _get_chat_listener()
    if _is_irc_mode():
        return '' if listener.is_channel_connected(user.get('twitch_user_id', '')) else _irc_connection_warning()

    try:
        await listener.ensure_channel_subscription(user['twitch_user_id'])
    except httpx.HTTPStatusError as exc:
        if exc.response.status_code == 403:
            return _subscription_warning(settings.twitch_bot_user_login or settings.irc_bot_login or 'your_bot')
        if exc.response.status_code == 400:
            return _subscription_setup_warning(settings.twitch_bot_user_login or settings.irc_bot_login or 'your_bot')
        logger.warning(
        ...
        return 'Не удалось автоматически активировать чат. Попробуй ещё раз через пару секунд.'
...
async def home(request: Request):
    ...
    return templates.TemplateResponse(
        'home.html',
        {
            ...
            'bot_login': settings.twitch_bot_user_login or settings.irc_bot_login or 'your_bot',
        },
    )
...
async def twitch_callback(request: Request, code: str = '', state: str = ''):
    ...
    try:
        ...
        if exc.response.status_code == 403:
            warning = _subscription_warning(settings.twitch_bot_user_login or settings.irc_bot_login or 'your_bot')
        elif exc.response.status_code == 400:
            warning = _subscription_setup_warning(settings.twitch_bot_user_login or settings.irc_bot_login or 'your_bot')
        else:
            raise
    ...
    return templates.TemplateResponse(
        'dashboard.html',
        {
            ...
            'chat_status_text': '' if chat_connected else _bot_not_moderator_text(
                settings.twitch_bot_user_login or settings.irc_bot_login or 'your_bot'
            ),
...
async def update_dashboard_settings(
    ...
    normalized_chat_questions_enabled = bool(user.get('chat_questions_enabled', 0)) if chat_questions_enabled is None else chat_questions_enabled == '1'
    normalized_chat_correct_answers_enabled = bool(user.get('chat_correct_answers_enabled', 0)) if chat_correct_answers_enabled is None else chat_correct_answers_enabled == '1'
    normalized_chat_winners_enabled = bool(user.get('chat_winners_enabled', 0)) if chat_winners_enabled is None else chat_winners_enabled == '1'
